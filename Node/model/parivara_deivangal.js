...
const parivaraDeivangalSchema = Joi.object({
...
const ParivaraDeivangalController = (function () {

  const _createParivaraDeivangal = async (jsonValue, callback) => {
    const { error, value } = parivaraDeivangalSchema.validate(jsonValue);
    ...
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        try {
          const insertQuery = `
          INSERT INTO parivara_deivangal
          (created_date, deity_speciality, deivam_type, fastings, name, notes, photo, prayer_request, separate_sannidhi, special_days, special_name, special_puja, status, updated_date, worship_method, temple_id, relationship)
          VALUES
          (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        `;
          ...
          callback(null, {
            ...
            code: 200,
            message: "Parivara Deivangal created successfully",
  ...
  const _getParivaraDeivangal = async (callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
  ...
  const _getParivaraDeivangalById = async (id, callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
        ...
        try {
          ...
          if (rows.length === 0) return callback({ result: result_failure, code: 404, message: "Parivara Deivangal not found" }, null);
  ...
  const _getParivaraDeivangalByTempleId = async (temple_id, callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
  ...
  const _updateParivaraDeivangal = async (jsonValue, callback) => {
    const { error, value } = parivaraDeivangalSchema.validate(jsonValue);
    ...
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        try {
          const checkQuery = `SELECT id FROM parivara_deivangal WHERE id = ?`;
          const [checkResult] = await client.query(checkQuery, [value.id]);
          if (checkResult.length === 0) return callback({ result: result_failure, code: 404, message: "Parivara Deivangal not found" }, null);

          const updateQuery = `
            UPDATE parivara_deivangal
            SET deity_speciality = ?, deivam_type = ?, fastings = ?, name = ?, notes = ?, 
                photo = COALESCE(?, photo), 
                prayer_request = ?, separate_sannidhi = ?, special_days = ?, special_name = ?, 
                special_puja = ?, status = ?, updated_date = ?, worship_method = ?, temple_id = ?, 
                relationship = ?
            WHERE id = ?
          `;
          ...
          callback(null, { result: result_success, code: 200, message: "Parivara Deivangal updated successfully" });
        } catch (queryError) {
          callback({ result: result_failure, code: 400, message: queryError.message }, null);
        } finally {
          client.release();
  ...
  const _deleteParivaraDeivangal = async (jsonValue, callback) => {
    ...
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        try {
          ...
          const deleteQuery = `DELETE FROM parivara_deivangal WHERE id = ?`;
          await client.query(deleteQuery, [value.id]);

          callback(null, { result: result_success, code: 200, message: "Parivara Deivangal deleted successfully" });
        } catch (queryError) {
          callback({ result: result_failure, code: 400, message: queryError.message }, null);
        } finally {
          client.release();
  ...
  return {
    CreateParivaraDeivangal: _createParivaraDeivangal,
    GetParivaraDeivangal: _getParivaraDeivangal,
    GetParivaraDeivangalById: _getParivaraDeivangalById,
    GetParivaraDeivangalByTempleId: _getParivaraDeivangalByTempleId,
    UpdateParivaraDeivangal: _updateParivaraDeivangal,
    DeleteParivaraDeivangal: _deleteParivaraDeivangal,
  };
})();

module.exports = ParivaraDeivangalController;
