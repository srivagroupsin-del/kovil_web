...
const ubaDeivangalSchema = Joi.object({
...
const UbaDeivangalController = (function () {

  const _createUbaDeivangal = async (jsonValue, callback) => {
    const { error, value } = ubaDeivangalSchema.validate(jsonValue);
      ...
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        `;
          ...
          callback(null, {
            ...
            code: 200,
            message: "Uba Deivangal created successfully",
  ...
  const _getUbaDeivangal = async (callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
  ...
  const _getUbaDeivangalById = async (id, callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
        ...
        try {
          ...
          if (rows.length === 0) return callback({ result: result_failure, code: 404, message: "Uba Deivangal not found" }, null);
  ...
  const _getUbaDeivangalByTempleId = async (temple_id, callback) => {
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        if (err) return callback({ result: result_failure, code: 500, message: "Database connection error" }, null);
  ...
  const _updateUbaDeivangal = async (jsonValue, callback) => {
    const { error, value } = ubaDeivangalSchema.validate(jsonValue);
    ...
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        try {
          const checkQuery = `SELECT id FROM uba_deivangal WHERE id = ?`;
          const [checkResult] = await client.query(checkQuery, [value.id]);
          if (checkResult.length === 0) return callback({ result: result_failure, code: 404, message: "Uba Deivangal not found" }, null);

          const updateQuery = `
            UPDATE uba_deivangal
          ...
          callback(null, { result: result_success, code: 200, message: "Uba Deivangal updated successfully" });
        } catch (queryError) {
          callback({ result: result_failure, code: 400, message: queryError.message }, null);
        } finally {
          client.release();
  ...
  const _deleteUbaDeivangal = async (jsonValue, callback) => {
    ...
    try {
      getConnectionWsinventoryPool(async (err, client) => {
        ...
        try {
          ...
          const deleteQuery = `DELETE FROM uba_deivangal WHERE id = ?`;
          await client.query(deleteQuery, [value.id]);

          callback(null, { result: result_success, code: 200, message: "Uba Deivangal deleted successfully" });
        } catch (queryError) {
          callback({ result: result_failure, code: 400, message: queryError.message }, null);
        } finally {
          client.release();
  ...
  return {
    CreateUbaDeivangal: _createUbaDeivangal,
    GetUbaDeivangal: _getUbaDeivangal,
    GetUbaDeivangalById: _getUbaDeivangalById,
    GetUbaDeivangalByTempleId: _getUbaDeivangalByTempleId,
    UpdateUbaDeivangal: _updateUbaDeivangal,
    DeleteUbaDeivangal: _deleteUbaDeivangal,
  };
})();

module.exports = UbaDeivangalController;
