...
const KavalDeivangalController = require("../model/kaval_deivangal");
...
router.post(["/create", "/save"], authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("KavalDeivangal", "Create"), async (req, res) => {
  ...
  try {
    ...
    KavalDeivangalController.CreateKavalDeivangal(params, (error, result) => {
...
router.get("/", authenticate, async (req, res) => {
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    KavalDeivangalController.GetKavalDeivangal((error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/temple/:temple_id", authenticate, async (req, res) => {
  ...
  try {
    ...
    KavalDeivangalController.GetKavalDeivangalByTempleId(temple_id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    KavalDeivangalController.GetKavalDeivangalById(id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.put("/update/:id", authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("KavalDeivangal", "Update"), async (req, res) => {
  const url = req.originalUrl;
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    KavalDeivangalController.UpdateKavalDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.delete("/:id", authenticate, activityTimelineMiddleware("KavalDeivangal", "Delete"), async (req, res) => {
  const url = req.originalUrl;
  const params = { id: req.params.id };
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    KavalDeivangalController.DeleteKavalDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
