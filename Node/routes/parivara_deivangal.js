...
const ParivaraDeivangalController = require("../model/parivara_deivangal");
...
router.post(["/create", "/save"], authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("ParivaraDeivangal", "Create"), async (req, res) => {
  ...
  try {
    ...
    ParivaraDeivangalController.CreateParivaraDeivangal(params, (error, result) => {
...
router.get("/", authenticate, async (req, res) => {
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    ParivaraDeivangalController.GetParivaraDeivangal((error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/temple/:temple_id", authenticate, async (req, res) => {
  ...
  try {
    ...
    ParivaraDeivangalController.GetParivaraDeivangalByTempleId(temple_id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    ParivaraDeivangalController.GetParivaraDeivangalById(id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.put("/update/:id", authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("ParivaraDeivangal", "Update"), async (req, res) => {
  const url = req.originalUrl;
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    ParivaraDeivangalController.UpdateParivaraDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.delete("/:id", authenticate, activityTimelineMiddleware("ParivaraDeivangal", "Delete"), async (req, res) => {
  const url = req.originalUrl;
  const params = { id: req.params.id };
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    ParivaraDeivangalController.DeleteParivaraDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
