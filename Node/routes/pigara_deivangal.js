...
const PigaraDeivangalController = require("../model/pigara_deivangal");
...
router.post(["/create", "/save"], authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("PigaraDeivangal", "Create"), async (req, res) => {
  ...
  try {
    ...
    PigaraDeivangalController.CreatePigaraDeivangal(params, (error, result) => {
...
router.get("/", authenticate, async (req, res) => {
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    PigaraDeivangalController.GetPigaraDeivangal((error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/temple/:temple_id", authenticate, async (req, res) => {
  ...
  try {
    ...
    PigaraDeivangalController.GetPigaraDeivangalByTempleId(temple_id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.get("/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    PigaraDeivangalController.GetPigaraDeivangalById(id, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.put("/update/:id", authenticate, (req, res, next) => {
...
}, activityTimelineMiddleware("PigaraDeivangal", "Update"), async (req, res) => {
  const url = req.originalUrl;
  ...
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    PigaraDeivangalController.UpdatePigaraDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
...
router.delete("/:id", authenticate, activityTimelineMiddleware("PigaraDeivangal", "Delete"), async (req, res) => {
  const url = req.originalUrl;
  const params = { id: req.params.id };
  try {
    winston.info(JSON.stringify({ url, params, data: params }));
    PigaraDeivangalController.DeletePigaraDeivangal(params, (error, result) => {
      if (!error) res.json(result);
      else res.status(error.code || 400).json(error);
    });
  } catch (err) {
