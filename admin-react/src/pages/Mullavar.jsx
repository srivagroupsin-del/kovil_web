...
const API_BASE_URL = BASE_API + '/moolavar';
...
const Mullavar = () => {
  ...
  const handlePhotoFilesChange = (e) => {
    ...
    if (remaining <= 0) {
      showWarning("அதிகபட்சம் 5 புகைப்படங்கள்", "You can upload a maximum of 5 photos!");
      return;
    }
  ...
  const handleRemovePhotoPreview = (id) => {
  ...
  const clearForm = () => {
    ...
    setChariotPreview('');
    setEditingId(null);
    const fileInputs = ['photoInput', 'utsavarInput', 'chariotInput'];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        // Convert camelCase to snake_case if necessary, or just use the keys as they are if backend expects them
        // Looking at your submitData, most keys are mapped
      });

      // Mapping as per your original submitData
      formDataObj.append('status', formData.status);
      formDataObj.append('name', formData.name);
      formDataObj.append('deity_name', formData.deityName);
      formDataObj.append('special_name', Array.isArray(formData.specialName) ? formData.specialName.filter(s => s.trim() !== '').join(', ') : formData.specialName);
      formDataObj.append('deity_form', Array.isArray(formData.deityForm) ? formData.deityForm.filter(s => s.trim() !== '').join(', ') : formData.deityForm);
      formDataObj.append('form_speciality', Array.isArray(formData.formSpeciality) ? formData.formSpeciality.filter(s => s.trim() !== '').join(', ') : formData.formSpeciality);
      formDataObj.append('divine_speciality', formData.divineSpeciality);
      formDataObj.append('vehicle', formData.vehicle);
      formDataObj.append('temple_tree', formData.templeTree);
      formDataObj.append('preferred_flower', formData.preferredFlower);
      formDataObj.append('vimana_level', formData.vimanaLevel);
      formDataObj.append('worship_time', formData.worshipTime);
      formDataObj.append('utsavar_photo', formData.utsavarPhoto);
      formDataObj.append('chariot_photo', formData.chariotPhoto);
      formDataObj.append('balipitham', formData.balipitham);
      formDataObj.append('kodimaram', formData.kodimaram);
      formDataObj.append('temple_id', CURRENT_TEMPLE_ID);

      // Append Moolavar photos
      const existingPhotos = photoPreviews.filter(p => p.isExisting).map(p => p.filename).join(', ');
      formDataObj.append('existing_photos', existingPhotos);

      photoPreviews.filter(p => !p.isExisting).forEach(p => {
        if (p.file) {
          formDataObj.append('photo', p.file);
        }
      });

      if (utsavarFile) formDataObj.append('utsavar_file', utsavarFile);
      else if (isEditMode && utsavarPreview && !utsavarPreview.startsWith('data:')) {
        formDataObj.append('utsavar_file', utsavarPreview.split('/').pop());
      }

      if (chariotFile) formDataObj.append('chariot_file', chariotFile);
      else if (isEditMode && chariotPreview && !chariotPreview.startsWith('data:')) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          specialName: json.special_name ? json.special_name.split(',').map(s => s.trim()) : [''],
          deityForm: json.deity_form ? json.deity_form.split(',').map(s => s.trim()) : [''],
          formSpeciality: json.form_speciality ? json.form_speciality.split(',').map(s => s.trim()) : [''],
        ...
        if (json.photo) {
          const files = json.photo.split(',').map(s => s.trim()).filter(Boolean);
          const previews = files.map((filename, index) => ({
            id: `existing-${index}`,
            previewUrl: `${BASE_API}/files/${filename}`,
            file: null,
            isExisting: true,
            filename
          }));
    ...
    <div className="form-card" style={{ maxWidth: '1400px' }}>
      <div className="form-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
          ...
          <h2 style={{ margin: 0 }}>மூலவர் விவரங்கள் (Moolavar Details)</h2>
        </div>
        <p>கோவிலின் மூலவர் பற்றிய அனைத்து தகவல்களையும் இங்கே நிர்வகிக்கலாம்.</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
          ...
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '700' }}>
            {editingId ? 'மூலவர் விவரத்தை மாற்ற (Edit Moolavar)' : 'புதிய மூலவர் பதிவு (Add New Moolavar)'}
          </h3>
        </div>
        ...
        <div className="form-grid">
          ...
          <FormInput label="மூலவர் பெயர்" name="name" value={formData.name} onChange={handleInputChange} icon={User} placeholder="பெயர்..." />
          <FormInput label="திருநாமம்" name="deityName" value={formData.deityName} onChange={handleInputChange} icon={User} placeholder="திருநாமம்..." />
          <DynamicInputList 
            label="சிறப்பு பெயர்" 
            name="specialName" 
            values={formData.specialName} 
            onChange={handleArrayChange} 
            icon={Info} 
            placeholder="சிறப்பு பெயர்..." 
          />
          <DynamicInputList 
            label="திருவுருவம்" 
            name="deityForm" 
            values={formData.deityForm} 
            onChange={handleArrayChange} 
            icon={Layers} 
            placeholder="திருவுருவம்..." 
          />
          <DynamicInputList 
            label="திருவுருவச் சிறப்பு" 
            name="formSpeciality" 
            values={formData.formSpeciality} 
            onChange={handleArrayChange} 
            icon={Info} 
            placeholder="சிறப்பு..." 
          />
          <FormInput label="திருநாமச் சிறப்பு" name="divineSpeciality" value={formData.divineSpeciality} onChange={handleInputChange} icon={History} placeholder="சிறப்புக்கள்..." />
          ...
          <div className="form-group" style={{ gridColumn: 'span 3' }}>
            <div className="form-grid" style={{ padding: 0 }}>
              <div className="form-group">
                <label className="form-label">மூலவர் புகைப்படம் (Moolavar Photos - Max 5)</label>
                <div style={{ border: '2px dashed #e2e8f0', borderRadius: '12px', padding: '16px', textAlign: 'center', background: '#f8fafc', minHeight: '120px' }}>
                  <input type="file" id="photoInput" accept="image/*" multiple onChange={handlePhotoFilesChange} style={{ display: 'none' }} />
                  <label htmlFor="photoInput" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    <Camera size={24} color="#94a3b8" style={{ marginBottom: '4px' }} />
                    <p style={{ margin: 0, fontSize: '12px', color: '#64748b' }}>Choose Photos (Max 5)</p>
                  </label>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '12px', justifyContent: 'center' }}>
                      {photoPreviews.map((item, idx) => (
                        <div key={item.id} style={{ position: 'relative', width: '50px', height: '50px' }}>
                          <img src={item.previewUrl} alt={`Preview ${idx}`} style={{ width: '50px', height: '50px', objectFit: 'cover', borderRadius: '6px', border: '1px solid #e2e8f0' }} />
                          <button
                            type="button"
                            onClick={() => handleRemovePhotoPreview(item.id)}
                            style={{
                              position: 'absolute',
                              top: '-4px',
                              right: '-4px',
      ...
      <div className="card" style={{ padding: 0 }}>
        <div style={{ padding: '24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '700' }}>மூலவர் பட்டியல் (Moolavar List)</h3>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setViewModalData(null)}>
          <div className="modal-content" style={{ maxWidth: '800px' }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3 style={{ margin: 0 }}>மூலவர் விவரம் (Moolavar Details)</h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '32px' }}>
                <div>
                  <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                    <p style={{ fontWeight: '700', marginBottom: '8px', color: '#64748b', fontSize: '12px', textTransform: 'uppercase' }}>மூலவர் புகைப்படம்</p>
                    {viewModalData.photo ? (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '8px', marginBottom: '16px' }}>
                        {viewModalData.photo.split(',').map(s => s.trim()).filter(Boolean).map((filename, idx) => (
                          <img key={idx} src={`${BASE_API}/files/${filename}`} style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', borderRadius: '8px', border: '1px solid #e2e8f0' }} alt={`Moolavar ${idx + 1}`} />
                        ))}
                      </div>
                    ) : (
                      <div style={{ width: '100%', aspectRatio: '1', background: '#f1f5f9', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                     <div>
                        <p style={{ fontWeight: '700', marginBottom: '4px', color: '#64748b', fontSize: '11px', textTransform: 'uppercase' }}>உற்சவர்</p>
                     ...
                     <div>
                        <p style={{ fontWeight: '700', marginBottom: '4px', color: '#64748b', fontSize: '11px', textTransform: 'uppercase' }}>தேர்</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  {[
                    ...
                    <div key={detail.label}>
                      <p style={{ margin: 0, fontSize: '11px', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <detail.icon size={12} /> {detail.label}
                      </p>
                      <p style={{ margin: 0, fontSize: '14px', fontWeight: '600', color: '#1e293b' }}>{detail.value || '-'}</p>
                    </div>
                  ))}
                  <div style={{ gridColumn: 'span 2' }}>
                    <p style={{ margin: 0, fontSize: '11px', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase' }}>சிறப்புக்கள்</p>
                    <p style={{ margin: 0, fontSize: '13px', color: '#475569' }}>{viewModalData.divineSpeciality || 'தகவல் இல்லை'}</p>
                  </div>
                </div>
              </div>
            </div>
