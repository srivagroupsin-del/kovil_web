...
const API_BASE_URL = BASE_API + '/visucam_neram';
...
const VisucamNeram = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      form.append('vishesha_name', formData.viseshaName);
      form.append('pooja_name', formData.poojaName);
      form.append('poojai_title', formData.poojaiTitle);
      form.append('eng_date', formData.engDate);
      ...
      form.append('viratagal', formData.viratagal);
  ...
  const filteredData = data.filter(item => 
    item.vishesha_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="form-card" style={{ maxWidth: '1400px' }}>
      <div className="form-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
          ...
          <h2 style={{ margin: 0, fontSize: '20px' }}>விஷூசம் (விசேஷங்கள்)</h2>
        </div>
        <p style={{ fontSize: '13px' }}>கோவிலின் விசேஷ தினங்கள் மற்றும் விசேஷ நேரங்களை நிர்வகிக்கவும்.</p>
      </div>

      <div className="card" style={{ marginBottom: '24px', padding: '20px' }}>
        <form onSubmit={handleSave}>
          <div className="form-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
            <FormInput label="விசேஷ பெயர்" name="viseshaName" value={formData.viseshaName} onChange={handleInputChange} icon={Zap} required />
            <FormInput label="பூஜை பெயர்" name="poojaName" value={formData.poojaName} onChange={handleInputChange} icon={Sparkles} required />
            <FormInput label="பூஜையின் தலைப்பு" name="poojaiTitle" value={formData.poojaiTitle} onChange={handleInputChange} icon={Activity} />
            ...
            <FormInput label="விரதகள்" name="viratagal" value={formData.viratagal} onChange={handleInputChange} icon={Info} />
            <FormSelect 
            ...
            <div className="form-group">
              <label className="form-label">விசேஷ புகைப்படம்</label>
              <div 
      ...
      <div className="card" style={{ padding: 0 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '700' }}>விஷூசம் பட்டியல்</h3>
          </div>
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: '600', fontSize: '13.5px' }}>{item.vishesha_name}</div>
                          <div style={{ fontSize: '11px', color: '#64748b' }}>{item.pooja_name || '-'}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div style={{ fontWeight: '600', fontSize: '13px' }}>{item.eng_date?.split('T')[0]}</div>
