...
const API_BASE_URL = BASE_API + '/kilamai_poojas';
...
const WeekPooja = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Error fetching week pooja data:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    if (checked) newVagai.push(value);
    else newVagai = newVagai.filter(v => v !== value);
  ...
  const handleSave = async (e) => {
    ...
    if (!formData.day || !formData.poojaTime || !formData.poojaName || !formData.session) {
      showWarning("தேவையான புலங்களை நிரப்பவும்", "Please fill required fields!");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showError("தோல்வி", "Failed to save week ritual");
      }
    } catch (err) {
    ...
    <div className="form-card" style={{ maxWidth: '1400px' }}>
      <div className="form-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
          ...
          <h2 style={{ margin: 0 }}>கிழமை பூஜைகள் (Weekly Rituals)</h2>
        </div>
        <p>மூலவருக்கு உரிய வாராந்திர பூஜைகள் மற்றும் கால நேரங்களை இங்கே நிர்வகிக்கலாம்.</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
          ...
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '700' }}>
            {editingId ? 'கிழமை பூஜை விவரத்தை மாற்ற (Edit Weekly Ritual)' : 'புதிய கிழமை பூஜை பதிவு (Add New Weekly Ritual)'}
          </h3>
        </div>
        ...
        <div className="form-grid">
          ...
          <FormInput label="அபிஷேகம் (Abhishekam)" name="abhishekam" value={formData.abhishekam} onChange={handleInputChange} icon={Info} placeholder="அபிஷேக விவரங்கள்..." />
          <FormInput label="தீர்த்தம் (Theertham)" name="theertham" value={formData.theertham} onChange={handleInputChange} icon={Info} placeholder="தீர்த்த விவரங்கள்..." />
        ...
        <div className="form-actions">
          ...
          <button onClick={handleSave} className="btn btn-primary">
            {editingId ? <>Update Ritual <RefreshCw size={18} /></> : <>Save Weekly Ritual <Save size={18} /></>}
          </button>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div style={{ padding: '24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '700' }}>கிழமை பூஜைகள் பட்டியல் (Weekly Ritual List)</h3>
          </div>
